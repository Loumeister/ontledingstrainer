...
export const TrainerScreen: React.FC<TrainerScreenProps> = ({
  ...
  useEffect(() => {
    if (!validationResult) {
      setFeedbackNudgeDismissed(false);
    }
  }, [validationResult]);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-2 md:p-4 font-sans transition-colors duration-300 flex flex-col">
      ...
      <main className="max-w-6xl mx-auto w-full flex flex-col gap-4 flex-1 mb-20">
        ...
        <div className="space-y-4 flex-1 flex flex-col">
          ...
          {validationResult && !validationResult.isPerfect && !showAnswerMode && !feedbackNudgeDismissed && (
            <div className="fixed inset-0 z-[700] bg-slate-900/30 flex items-end md:items-center justify-center p-3">
              <div className="w-full max-w-xl rounded-2xl border-2 border-amber-300 dark:border-amber-700 bg-white dark:bg-slate-800 shadow-2xl p-4 md:p-5 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <h3 className="text-base md:text-lg font-extrabold text-slate-900 dark:text-white">Even kort naar je feedback 👀</h3>
                  <button
                    onClick={() => setFeedbackNudgeDismissed(true)}
                    className="px-2 py-1 text-xs rounded-md text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700"
                  >Sluiten</button>
                </div>
                <p className="text-sm text-slate-700 dark:text-slate-200 mb-3">
                  Je hebt {validationResult.score} van de {validationResult.total} goed. Bekijk minimaal 1 feedbackpunt en probeer dan opnieuw.
                </p>
                <div className="text-sm bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2 text-amber-900 dark:text-amber-200 mb-4">
                  Tip: begin met het blokje met een <strong>foute rol</strong> en check eerst of het een werkwoord is of juist extra informatie geeft.
                </div>
                <div className="flex justify-end">
                  <button
                    onClick={() => setFeedbackNudgeDismissed(true)}
                    className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-lg"
                  >Ik ga verbeteren</button>
                </div>
              </div>
            </div>
          )}
