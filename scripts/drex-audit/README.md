...
python3 -m unittest discover -s scripts/drex-audit    # tests zonder netwerk
```
...
Een antwoord telt als goed als de app het zou goedkeuren. Dat is dezelfde regel als `getConsistentRole` in `src/logic/validation.ts`: elk woord van het zinsdeel moet die rol of die `alternativeRole` hebben. De `alternativeRole: nwd` op "worden" in zin 147 is een alternatief op subrolniveau en telt dus niet voor het zinsdeel.
